...
import { AlertCircle, Heart, MessageSquare, Paperclip, Send, ThumbsUp, X, Video, CheckCircle, Clock, Upload } from "lucide-react";

export default function ChatInterface({ userRole = "client" }) { // Added userRole prop
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: "pinned",
      content: "Project brief: 2-min promo video, cinematic style",
      time: "10:00 AM",
    ...
    },
    {
      id: 5,
      type: "editor",
      content: "I'll try a few options and include them in the next draft.",
      time: "10:45 AM",
      replyTo: 4,
  ...
  const handleSendMessage = () => {
    ...
    const newMsg = {
      ...
      type: userRole, // Dynamic based on user role
      content: newMessage,
    ...
    setMessages(
      messages.map((message) => {
        if (message.id === messageId) {
          const reactions = message.reactions || []; // Fallback for safety
  ...
  // Sample milestones for project management
  const milestones = [
        ...
        {userRole === "editor" ? (
          <>
            ...
            <div className="space-y-2">
              ...
              <button className="mt-2 w-full p-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 flex items-center justify-center">
                <Upload size={18} className="mr-2" /> Upload Draft
              </button>
              <button className="mt-2 w-full p-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 flex items-center justify-center">
