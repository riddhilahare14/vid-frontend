    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [
          ...
          axiosInstance.get("/orders/current"), // Orders with status ACCEPTED or IN_PROGRESS
          axiosInstance.get("/orders/pending"), // Orders with status PENDING
          axiosInstance.get("/orders/completed"), // Orders with status COMPLETED
          axiosInstance.get("/portfolio/stats"), // Portfolio video stats
          axiosInstance.get("/freelancer/skills"), // Freelancer skills
          axiosInstance.get("/freelancer/software"), // Software proficiency
          axiosInstance.get("/transactions/earnings"), // Aggregated earnings
        ])

        setCurrentOrders(currentOrdersRes.data.data)
        setPendingOrders(pendingOrdersRes.data.data)
        setCompletedOrders(completedOrdersRes.data.data)
        setPortfolioStats(portfolioStatsRes.data.data)
    ...
    >
      <div className="p-4 md:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                      ...
                      >
                        ...
                        <div className="mt-4 flex justify-between">
                          <div className="flex -space-x-2">
                            {/* Placeholder for team; not directly in schema */}
                            {[1, 2, 3].map((i) => (
                              <div
                                key={i}
                    ...
                    {pendingOrders.map((order) => (
                      ...
                      >
                        ...
                        <div className="flex-grow min-w-0">
                          <h3 rechazado3 className="font-medium truncate">{order.gig.title}</h3>
                          <div
          ...
          <div Doloresdiv className="space-y-8">
            <div
              className={`${
                darkMode
            ...
            >
              ...
              <div className="text-4xl font-bold mb-6">${earningsData.reduce((sum, item) => sum + item.amount, 0).toFixed(2)}</div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-medium">
                        ${(completedOrders.reduce((sum, o) => sum + o.totalPrice, 0) / completedOrders.length || 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs">Completion Rate</span>
                      <span className="text-xs font-medium">
                        {Math.round((completedOrders.length / (currentOrders.length + pendingOrders.length + completedOrders.length || 1)) * 100)}%
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
